...
class Child extends Parent {
	...
	void ownMethod() {
		System.out.println("자식만이 메서드 입니다.");
	}
}
...
public class A_Class_Type {
	public static void main(String[] args) {
		...
		// 클래스의 자동 타입 변환 구현 방법
		// 1. 자식 객체를 생성하면서 바로 부모타입으로 변환할 떄
		// 부모클래스(타입) 객체변수 = new 자식클래스();

		// 2. 만들어진 자식 객체를 부모 타입으로 변환할 때
		// 자식클래스(타입) 객체변수 = new 자식클래스();
		...
		p2.displayInfo();
//		자동 형변환의 경우, 해당 객체는 보존되고 사용만 부모 객체처럼 사용
...
//		(자식타입) 부모타입. 메서드();
		...
		// 2.자식 클래스의 멤버들에 대한 접근이 여러번 필요할 경우
//		자식 타입 변수에 담아서 사용
//		자식 타입 변수 = (자식타입)부모타입;
		
		Animal1 genericAnimal = new Dog1();
		genericAnimal.display();
		
		Dog1 specificDog = (Dog1)genericAnimal;
		specificDog.bark();
	}
