...
// : 인터페이스 :  클래스를 동작을 정의하는 "계약" 역할, 다중 상속 효과
// - 객체가 어떤 행위를 해야하는지를 정의
// - extends 사용하여 인터페이스 간에 다중 구현
// - 추상 메서드, 상수만 포함
// - 구현된 것은 아무것도 없고 밑그림만 그려진 '기본 설계도'

// 추상클래스 : 관련된 클래스들의 공통적인 특징을 추출하여 '일부' 구현을 포함한 상태 제공
// extends 를 사용하여 단일 상속만 가능
// 추상 메서드, 구체 미서드, 필드, 생성자 포함 가능

// 인터페이스 선언 방법
// : 클래스와 작성하는 방법이 유사하지만 키워드 class 대신 interface를 사용
// : interface는 접근제어자로 public, default만 사용가능


// interface 인터페이스명{
//		1. 멤버 변수
//		interface가 가지는 필드는 모든 public static final 특성을 가짐
...
interface PlayingCard{
	...
	// 디폴트 메서드
	// : 인터페이스에 새 기능을 추가하면선 기존 구현을 포함
	default void defaultMethod() {
		System.out.println("디폴트 메서드 입니다");
	}
	// 정적 메서드
	// : 인터페이스에서 직접 구현 가능
	// - 인터페이스 이름으로 직접 호출 가능 , 오버라이딩 불가능
	static void staticMethod() {
		System.out.println("정적메서드 입니다.");
	}
}

// 인터페이스의 구현
// : 클래스 implements 키워드를 사용하여 인터페이스 구현
class Card implements PlayingCard{
//		인터페이스 안의 메서드를 구혆나 클래스는 public 지정
	@Override
