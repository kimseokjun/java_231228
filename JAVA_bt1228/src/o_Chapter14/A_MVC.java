...
// : 사용자의 입력에 반응하여 모델을 업데이트 , 필요한 경우 뷰를 새롭게 출력(재렌더링)


// MVC 패턴의 장점

// -  관심사를 분리 : 각 구성요소는 독립적인 가능과 책임 > 코드의 관리와 확자잉 용이
// - 유지보수의 용이성 : 애플리케이션의 비즈니스 로직과 사용자 인터페이스가 분리되어, 유지보수가 용이하다

// MVC 패턴의 단점
// - 복잡성 증가 : 간단한 애플리케이션 구현에는 MV 패턴의 구조가 복잡성을 증가
// - 학습 곡선이 높음 : 이해하고 사용하기 까지 일정한 학습이 필요하다.

// MVC 패턴의 작동 원리
// V > C > M > C > V
// 사용자 인터렉션 : 뷰를 통해 상호작용
// 컨트롤러 처리 : 사용자의 행동을 컨트롤러로 전달
...
public class A_MVC {
	public static void main(String[] args) {
		...
		// BookView : 사용자의 입력을 Controller에게 전달
		//							Controller에서 받은 정보를 출력
		
		// Controller ( 사용자의 입력을 받아 Model과 View 사이에서 중재)
		// BookController : 사용자의 입력에 따라 적절한 Model 메서드 출력
